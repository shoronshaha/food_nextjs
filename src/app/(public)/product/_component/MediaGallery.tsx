...
export default function MediaGallery({
  ...
  return (
    <div className="w-full md:mt-2">
      ...
      {!isMobile ? (
        <div
          className={media.length === 1 ? "w-full" : "grid grid-cols-2 gap-4"}
        >
          {media.map((item, index) => {
            ...
            return (
              <div
                key={item._id}
                className={`relative ${
                  media.length === 1 ? "w-full h-[1000px]" : "w-full h-[600px]"
                } bg-organic-50 rounded-2xl overflow-hidden border border-organic-100 shadow-organic hover:shadow-organic-lg transition-shadow`}
              >
                {isLoading && (
                  <SkeletonComponent
                ...
                {!isVideo && (
                  <div className="relative w-full h-full">
                    <AtomImage
                      ...
                      onError={(e) => {
                        console.error(
                          `Failed to load image ${item._id}: ${imageSrc}`
                        );
                        setMediaLoading((prev) => ({
                          ...prev,
                          [item._id]: false,
                ...
                {isVideo && (
                  <video
                    ...
                    onError={() =>
                      console.error(
                        `Failed to load video ${item._id}: ${item.url}`
                      )
                    }
                  />
                )}
                  ...
                  {isVideo && (
                    <video
                      ...
                      onLoadedMetadata={() => {
                        ...
                        if (isActive)
                          videoRefs.current[item._id]?.play().catch(() => null);
                      }}
                      onClick={() => handleVideoPlay(item._id)}
                      onError={() =>
                        console.error(
          ...
          {media.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 w-9 h-9 bg-organic-600 hover:bg-organic-700 rounded-full text-white flex items-center justify-center shadow-lg"
                aria-label="Previous"
              >
              ...
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 w-9 h-9 bg-organic-600 hover:bg-organic-700 rounded-full text-white flex items-center justify-center shadow-lg"
                aria-label="Next"
              >
            ...
            {media.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  index === currentSlide ? "bg-organic-600" : "bg-white/70"
                }`}
              />
            ))}
          </div>
