...
export default function DynamicCategorySections({
  ...
  const map = new Map<
    string,
    { category: string; products: Product[]; totalCount: number }
  >();
  ...
  categorizedProducts.push(
    ...Array.from(map.values()).filter((group) => group.products.length > 0)
  );
  ...
  if (categorizedProducts.length === 0) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4 font-urbanist">
        <div className="text-center space-y-6 max-w-md mx-auto">
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-r from-primary/10 via-primary/10 to-primary/10 rounded-full blur-xl"></div>
            <div className="relative inline-flex items-center justify-center w-24 h-24 bg-gradient-to-br from-secondary/10 via-white to-secondary/5 backdrop-blur-sm rounded-full border border-secondary/30">
    ...
    <section className="py-12 space-y-16 font-urbanist">
      <div className="md:space-y-16 space-y-6">
        {categorizedProducts.map(
          ({ category, products: catProducts, totalCount }, sectionIndex) => {
            ...
            return (
              <section
                key={category}
                className="group/section bg-secondary/10 dark:bg-gray-900/50 relative overflow-hidden py-4 rounded-3xl"
              >
                <div className="absolute inset-0 bg-secondary/10 dark:bg-gray-900/50 rounded-3xl"></div>

                <div className="relative space-y-8 lg:container mx-auto px-2 md:px-0">
                  <div className="relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent rounded-3xl blur-3xl scale-110"></div>
                    <div className="relative flex flex-col md:flex-row items-center justify-between gap-6 md:p-8 p-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl border border-secondary/30 dark:border-gray-700 shadow-lg transition-all duration-500 hover:shadow-xl">
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-4">
                          <div className="flex items-center gap-2 px-4 py-2 bg-secondary/10 dark:bg-secondary-900/20 rounded-full border border-secondary/30">
                      ...
                      <div className="hidden md:block">
                        <Link
                          ...
                          className="group relative inline-flex items-center gap-2 px-8 py-3 bg-primary text-white font-bold text-sm rounded-full shadow-lg hover:shadow-primary/30 hover:scale-105 transform transition-all duration-300 ease-out overflow-hidden"
                        >
                          <span className="relative z-10 whitespace-nowrap uppercase tracking-wider">
                            View Full Menu
                          </span>
                          <svg
                      ...
                      <div className="md:hidden w-full">
                        <Link
                          href={`/category/${encodeURIComponent(category)}`}
                          className="w-full text-center group inline-flex items-center justify-center gap-2 px-5 py-3 bg-primary text-white font-bold text-sm rounded-full shadow-md hover:shadow-lg transition-all duration-300"
                        >
                          <span>See All</span>
                          <svg
