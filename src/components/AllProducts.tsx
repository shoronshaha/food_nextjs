...
export default function AllProducts({
  ...
  const [selected, setSelected] = useState<string>(() =>
    urlCondition && tabs.some((t) => t.id === urlCondition)
      ? urlCondition
      : "all"
  );
  ...
  const displayedProducts = useMemo(() => {
    ...
    const isLargeDevice =
      typeof window !== "undefined" && window.innerWidth >= 1024; // lg: 1024px and above
    const isMediumDevice =
      typeof window !== "undefined" &&
      window.innerWidth >= 768 &&
      window.innerWidth < 1024; // md: 768px to 1023px
    const isMobileDevice =
      typeof window !== "undefined" && window.innerWidth < 768; // below 768px
    ...
    <div className="px-1 pt-0 pb-4 md:px-4 md:pt-0 md:pb-6 bg-secondary dark:secondary">
      ...
      <div className="relative mb-6 md:mb-8 text-center">
        <div className="inline-flex gap-2 items-center mb-3 mt-16">
          <p className="text-gray-500 text-3xl dark:text-white font-urbanist">
            OUR{" "}
            <span className="text-primary font-bold dark:text-primary/80">
              MENU
            </span>
          </p>
          <p className="w-8 sm:w-12 h-0.5 bg-primary dark:bg-primary/80"></p>
        </div>
        <p className="mx-10 sm:mx-auto sm:w-1/2 text-xs sm:text-sm md:text-base text-gray-600 dark:text-gray-300">
          ...
          <span className="text-black dark:text-white">
            Explore our wide range of delicious dishes, crafted with love and
              ...
              <div className="inline-flex flex-col items-center gap-4 md:gap-6 max-w-md mx-auto px-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                    ...
                    {tabs.find((t) => t.id === selected)?.label} category.
                  </p>
                </div>
                <motion.button
