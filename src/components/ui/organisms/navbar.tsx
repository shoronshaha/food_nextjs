  ...
  const suggestions: SearchResultItem[] = useMemo(() => {
    ...
    const categoryMatches = categories
      ...
      .map((cat: any) => ({
        ...
        url: `/products?category=${cat._id}&name=${encodeURIComponent(
    ...
    <>
      ...
      <div
        ...
        className={`w-full z-[100] sticky top-0 transition-all duration-300 font-urbanist
          ${
            isScrolled
              ? "bg-white/80 backdrop-blur-md shadow-md py-2 border-b border-primary/20"
              : "bg-base py-4"
          } ${className || ""}`}
      >
        {/* Mobile Bottom Navigation Bar - UPDATED COLORS */}
        {!isProductsPage && !isCheckoutPage && !isProductDetailPage && (
          <div className="md:hidden fixed bottom-5 left-4 right-4 z-50 bg-secondary/90 backdrop-blur-lg border border-organic-100/50 rounded-2xl flex justify-around items-center h-16 shadow-2xl shadow-organic">
            ...
            <Link
              href="/"
              className="flex flex-col items-center text-xs text-primary font-bold"
            >
              <div className="bg-organic-50 p-2 rounded-full mb-1">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  strokeWidth="0"
                ...
                >
                  <path d="M12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2ZM12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4ZM12 5V19C15.9 19 19 15.9 19 12C19 8.1 15.9 5 12 5Z"></path>
                </svg>
              </div>
              <span>Home</span>
            </Link>
            ...
            <button
              ...
              className="flex flex-col items-center text-xs text-earth-700 hover:text-primary transition-colors"
            >
              <svg
                stroke="currentColor"
                fill="none"
                ...
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M11 21.886l-9.192-9.193a2.98 2.98 0 0 1 0-4.215l8.808-8.807a2.98 2.98 0 0 1 4.215 0l9.192 9.192a2.978 2.978 0 0 1 0 4.215l-8.808 8.807a2.98 2.98 0 0 1-4.215 0z"></path>
              </svg>
              <span>Our Menu</span>
            </button>
            <div className="flex flex-col items-center text-xs text-gray-600">
        ...
        <div className="hidden md:flex items-center justify-between container mx-auto px-6">
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-2 group">
              ...
              <div className="flex flex-col">
                ...
                <small className="text-xs text-primary/70 -mt-1">
                  Organic & Farm-Fresh
                </small>
              </div>
            </Link>
          ...
          <div className="flex items-center bg-secondary/50 backdrop-blur-sm border border-organic-100 rounded-full px-2 py-1 shadow-sm">
            <Link
              href="/"
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${
                pathname === "/"
                  ? "bg-primary text-white shadow-md"
                  : "text-earth-700 hover:text-primary hover:bg-secondary/10"
              }`}
            >
              Home
            </Link>
            <Link
              ...
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${
                pathname?.startsWith("/products")
                  ? "bg-primary text-white shadow-md"
                  : "text-earth-700 hover:text-primary hover:bg-organic-50"
              }`}
            >
              Full Menu
            </Link>
            <button
              onClick={toggle}
              className="px-5 py-2 rounded-full text-sm font-semibold text-gray-600 hover:text-primary hover:bg-secondary/10 flex items-center gap-2"
            >
              <SidebarToggler />
            ...
            <div className="relative" ref={searchRef}>
              <div
                className={`flex items-center gap-2 px-3 py-2 rounded-full transition-all ${
                  showSearchBar
                    ? "bg-white shadow-lg ring-2 ring-primary/20 w-64"
                    : "bg-white/50 hover:bg-white border border-transparent hover:border-secondary/20 w-auto"
                }`}
              >
                <button
                  type="button"
                ...
                {showSearchBar && (
                  <input
                    ...
                    placeholder="Search farm-fresh produce, meals and more..."
                    value={searchTerm}
                ...
                {!showSearchBar && (
                  <span
                    onClick={handleSearchClick}
                    className="text-sm font-medium text-gray-500 cursor-pointer hidden lg:block"
                  >
                    Search...
                  </span>
                )}
