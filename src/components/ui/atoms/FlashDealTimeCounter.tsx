...
export const FlashDealTimeCounter = React.memo(
  ({ endDate, variant = "default" }: FlashDealTimeCounterProps) => {
        ...
        <div className="relative group">
          ...
          <div className="relative bg-gradient-to-r from-gray-900 via-purple-900 to-gray-900 rounded-xl p-4 shadow-2xl border border-purple-500/30">
            ...
            <div className="absolute inset-0 overflow-hidden rounded-xl">
              ...
              <div className="absolute top-3 right-2 w-2 h-2 bg-primary/60 rounded-full animate-pulse opacity-40"></div>
              <div
                className="absolute bottom-2 left-6 w-3 h-3 bg-blue-400 rounded-full animate-bounce opacity-50"
            ...
            <div className="relative">
              <div className="text-center mb-3">
                <div className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-primary text-sm font-bold animate-pulse">
                  ⚡ FLASH SALE ENDS IN ⚡
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={item.label} className="flex items-center">
                    <div className="relative group/item">
                      <div
                        className={`absolute -inset-0.5 bg-gradient-to-r ${item.color} rounded-lg blur opacity-60 group-hover/item:opacity-100 transition animate-pulse`}
                      ></div>
                      <div className="relative bg-black/80 backdrop-blur-sm rounded-lg px-2 py-1 border border-white/20">
                        <div
                          className={`text-lg font-bold text-transparent bg-clip-text bg-gradient-to-b ${item.color} text-center`}
                        >
                    ...
                    {index < 3 && (
                      <div className="text-purple-400 mx-1 animate-pulse text-lg font-bold">
                        :
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-3 relative">
                <div className="h-1 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-red-500 via-yellow-500 to-green-500 animate-pulse rounded-full"
                    style={{ width: `${100 - timeLeft.days * 2}%` }}
                  ></div>
                </div>
              </div>
            </div>
    ...
    return (
      <div className="relative group">
        ...
        <div className="absolute inset-0 overflow-hidden rounded-2xl">
          <div className="absolute -top-1 -right-1 w-6 h-6 bg-gradient-to-r from-primary/60 to-purple-500 rounded-full opacity-30 animate-bounce"></div>
              ...
              {[
                {
                  ...
                  label: "D",
                  gradient:
                    "from-red-400 to-pink-500 text-black dark:text-white",
                },
                {
                  value: timeLeft.hours,
                  label: "H",
                  gradient:
                    "from-primary to-red-500 text-black dark:text-white",
                },
                {
                  value: timeLeft.minutes,
                  label: "M",
                  gradient:
                    "from-yellow-400 to-primary text-black dark:text-white",
                },
                {
                  value: timeLeft.seconds,
                  label: "S",
                  gradient:
                    "from-green-400 to-blue-500 text-black dark:text-white",
                },
              ].map((item, index) => (
                <div key={item.label} className="flex items-center">
                  {/* Time Box */}
                  <div className="relative group/box">
                    {/* Glow effect */}
                    <div
                      className={`absolute -inset-0.5 bg-gradient-to-r ${item.gradient} rounded-lg opacity-20 group-hover/box:opacity-40 blur transition-all animate-pulse`}
                    ></div>
                    ...
                    <div className="relative bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg border border-white/40 dark:border-gray-600/40 shadow-lg min-w-[28px] sm:min-w-[32px]">
                      <div className="px-1.5 py-1 text-center">
                        {/* Number */}
                        <div
                          className={`text-sm sm:text-base font-bold bg-gradient-to-b ${item.gradient} text-transparent bg-clip-text`}
                        >
                          {item.value.toString().padStart(2, "0")}
                        </div>
                        {/* Label */}
