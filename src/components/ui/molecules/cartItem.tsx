...
export function CartItem({
  ...
  const ProductImage = ({ size, sizes }: { size: string; sizes: string }) => (
    <div className="relative group/image flex-shrink-0 max-w-full">
      ...
      <div className="absolute -top-1 -right-1 bg-gradient-to-r from-pink-500 to-primary/50 text-white text-[9px] xs:text-[10px] sm:text-xs px-1 xs:px-1.5 sm:px-2 py-0.5 xs:py-1 rounded-full shadow-md transform rotate-12 group-hover:rotate-0 transition-transform duration-300">
        ৳{unitPrice}
      </div>
    </div>
  );
  ...
  const ProductInfo = ({ layout, totalClasses = "" }: { layout: string; totalClasses?: string }) => (
    <div className="flex-1 min-w-0 space-y-1">
      <h3
        className={`font-bold text-gray-800 dark:text-white leading-tight group-hover:text-primary dark:group-hover:text-primary/60 transition-colors duration-300 ${layout === 'mobile' ? 'text-sm ' : layout === 'tablet' ? 'text-base sm:text-lg' : 'text-lg xl:text-xl 2xl:text-2xl'} truncate pr-2`}
        title={item?.name}
      >
      ...
      <div className={`${layout === 'mobile' ? 'flex flex-col gap-1 mt-1' : layout === 'tablet' ? 'flex flex-wrap items-center gap-2 xs:gap-4 mt-2' : 'flex flex-col lg:flex-row lg:items-center gap-1 xs:gap-2 lg:gap-4 xl:gap-6 2xl:gap-8 mt-2'}`}>
        <div className="flex items-center gap-1 xs:gap-2">
          {item.isDiscountActive && item.sellingPrice && item.sellingPrice > unitPrice ? (
            <div className="flex items-center gap-2">
              <span className={`font-semibold text-primary dark:text-primary/60 ${layout === 'mobile' ? 'text-sm' : layout === 'tablet' ? 'text-sm xs:text-base' : 'text-base lg:text-lg xl:text-xl'}`}>
        ...
        {/* <div className="flex items-center gap-1 xs:gap-2">
          ...
          <div className={`bg-gradient-to-r from-primary/10 to-pink-500/10 dark:from-pink-400/10 dark:to-pink-500/10 px-2 xs:px-3 py-0.5 xs:py-1 rounded-full border border-primary/20 dark:border-pink-400/20 ${totalClasses}`}>
            <span className={`font-bold text-primary dark:text-primary/60 ${layout === 'mobile' ? 'text-sm xs:text-base' : layout === 'tablet' ? 'text-sm xs:text-lg' : 'text-lg xl:text-xl 2xl:text-2xl'}`}>
  ...
  const QuantityControls = ({ size }: { size: string }) => (
    <div className={`flex items-center bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 group-hover:border-primary dark:group-hover:border-primary/60 transition-all duration-300 ${size === 'desktop' ? 'rounded-xl border-2' : ''}`}>
      <button
        ...
        disabled={!canDec}
      >
        <FiMinus className={`text-gray-600 dark:text-gray-300 group-hover/dec:text-primary dark:group-hover/dec:text-primary/60 transition-all duration-200 ${size === 'mobile' ? 'w-3.5 h-3.5' : size === 'tablet' ? 'w-4 h-4' : 'w-4 h-4 group-hover/dec:scale-110'}`} />
      </button>
      ...
      >
        <FiPlus className={`text-gray-600 dark:text-gray-300 group-hover/inc:text-primary dark:group-hover/inc:text-primary/60 transition-all duration-200 ${size === 'mobile' ? 'w-3.5 h-3.5' : size === 'tablet' ? 'w-4 h-4' : 'w-4 h-4 group-hover/inc:scale-110'}`} />
      </button>
      ...
      <div className="relative p-2 md:p-4 mt-2">
        ...
        <div className="block sm:hidden">
          ...
          <div className="flex flex-row items-center justify-between gap-2 mt-2">
            ...
            <div className="bg-gradient-to-r from-primary/10 to-pink-500/10 dark:from-primary/60/10 dark:to-pink-500/10 px-2.5 py-1.5 rounded-lg border border-primary/20 dark:border-pink-400/20">
              <span className="font-bold text-primary dark:text-primary/60 text-sm xs:text-base">
                {formatCurrency(totalPrice, currency)}
              </span>
            </div>
          </div>
        ...
        <div className="hidden md:grid grid-cols-4 gap-2 items-center">
          ...
          <div className="relative flex justify-center md:justify-start">
            <div className="relative group/image">
              ...
              <div className="absolute -top-2 -right-2 bg-gradient-to-r from-pink-500 to-primary/50 text-white text-xs px-2 py-1 rounded-full shadow-lg transform rotate-12 group-hover:rotate-0 transition-transform duration-300 text-nowrap">
                {formatCurrency(unitPrice, currency)}
              </div>
            </div>
          </div>
          ...
          <div className="col-span-2 space-y-3">
            <div className="relative">
              <h3 className="font-bold text-lg xl:text-xl text-gray-800 dark:text-white leading-tight group-hover:text-primary dark:group-hover:text-primary/60 transition-colors duration-300">
                {truncateName(item?.name)}
              </h3>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4">
              <div className="flex items-center gap-2">
                {item.isDiscountActive && item.sellingPrice && item.sellingPrice > unitPrice ? (
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-primary dark:text-primary/60 text-lg text-nowrap">
                      {formatCurrency(unitPrice, currency)}
                    </span>
              ...
              <div className="bg-gradient-to-r from-primary/10 to-pink-500/10 dark:from-primary/60/10 dark:to-pink-500/10 px-3 py-1 rounded-full border border-primary/20 dark:border-primary/60/20 text-nowrap">
                {item.isDiscountActive && item.sellingPrice && item.sellingPrice > unitPrice ? (
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-primary dark:text-primary/60 text-sm">
                      {formatCurrency(totalPrice, currency)}
                    </span>
          ...
          <div className="flex flex-col items-end justify-end gap-4">
            <div className="relative group/qty">
              <div className="flex items-center bg-gray-50 dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-600 group-hover/qty:border-primary dark:group-hover/qty:border-primary/60 transition-all duration-300 overflow-hidden">
                <button
                  ...
                  disabled={!canDec}
                >
                  <FiMinus className="w-4 h-4 text-gray-600 dark:text-gray-300 group-hover/dec:text-primary dark:group-hover/dec:text-primary/60 group-hover/dec:scale-110 transition-all duration-200" />
                </button>
                ...
                <button
                  ...
                  disabled={!canInc || (!ignoreStockCap && qty >= (item?.maxStock ?? 10))}
                >
                  <FiPlus className="w-4 h-4 text-gray-600 dark:text-gray-300 group-hover/inc:text-primary dark:group-hover/inc:text-primary/60 group-hover/inc:scale-110 transition-all duration-200" />
                </button>
              </div>
